...
void ST7789_DrawImage(uint16_t x, uint16_t y, uint16_t w, uint16_t h, const uint16_t *data)
{
	...
	// ********** ???????????,??????? **********
	uint32_t pixel_num = (uint32_t)w * h; // ????
	uint8_t send_buf[2];                  // ??????????????(?Fill_Color??)
	for (uint32_t i = 0; i < pixel_num; i++)
	{
		// ??????:?????????(ST7789????)
		send_buf[0] = (data[i] >> 8) & 0xFF; // ???(??Fill_Color?color>>8)
		send_buf[1] = data[i] & 0xFF;        // ???(??Fill_Color?color&0xFF)
		ST7789_WriteData(send_buf, sizeof(send_buf)); // ?Fill_Color????????
	}
	// ********** ???? **********
